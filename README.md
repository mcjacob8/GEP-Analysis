...
These scripts can be found under different directories. For example, **Simu** contains scripts related to simualtion work, **Calibration** scripts related to GEM DB parameters, or under the main directory itself. The **output** directory is general where output pdfs, text files, or small root files are stored. Large root file outputs should be stored in volatile.

## 2. Processes:
Here is a list of notable scripts that perform important analysis tasks. Each script itself should have a text sticker at the beginning explaining it in more detail.
...
4. **GetElasticYield.C**: Look at elastic yeild, useful when checking improvements to SBS offline or SBS replay DB.
5. **Simu/CompareAPV.C** & **Simu/DigiAPVTime.C**: These scripts work together to generate output txt files containing APV timing offsets to go back into the simulation. This is a work inprogress.
